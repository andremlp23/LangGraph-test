...
import pandas as pd
import pdfplumber # <-- Importação adicionada
...
Your task is to extract all relevant technical specifications related strictly to steel elements, including:
- Structural steel elements (beams, columns, frames, trusses)
- Secondary steel (supports, brackets, connections)
- Steel-based cladding systems ONLY if explicitly metallic

Ignore:
- Concrete, timber, aluminum, or non-steel materials
- Reinforcements unless explicitly described as steel elements (e.g., steel plates, steel stiffeners)

Extraction rules:
- Extract maximum relevant technical detail (profiles, grades, standards, treatments, connections, geometry)
- Keep technical descriptors (e.g., S275, galvanized, welded), even if they include units or standard codes
- Do NOT extract standalone quantities (e.g., 10 units, 500kg), but keep values that are part of specifications (e.g., 10mm thickness)
...
Item definition:
- Use a concise standardized name (e.g., 'Beam', 'Column', 'Bracket', 'Steel Cladding Panel')

Edge cases:
- If steel is mentioned but no technical detail is provided, output:
  'Steel Element | Mentioned without technical specification'
- If no steel-related information exists, output exactly:
  'SEM AÇO NESTE BLOCO'
...
Never infer or invent data."""
    ...
    for i, chunk in enumerate(chunks):
        ...
        # PROMPT HUMANO (Corrigido com 'f' no início para formatar variáveis)
...
### RESULTADO DA EXTRAÇÃO (Linhas separadas por '|'):
Elemento de Aço | Especificação Técnica Completa"""
        
        try:
            res = llm.invoke([mensagem_sistema, HumanMessage(content=prompt_bloco)])
            resposta = res.content.strip()
            
            # FILTRO DESLIGADO PARA DEBUG: Guarda absolutamente tudo o que a IA disser
            notas_recolhidas.append(f"--- RESPOSTA DA IA BLOCO {i+1} ---\n{resposta}\n")
...
with col1:
    ...
    # Agora aceita Excel e PDF
    file_uploaded = st.file_uploader("Carrega o ficheiro do cliente", type=["xlsx", "xls", "pdf"])

with col2:
    st.subheader("2. Matriz de Auditoria")
    st.info("O modo 'Engenheiro de Aço' está ativo. A IA vai focar-se em S355, normas, tratamentos e afins.")
...
if st.button("🚀 Iniciar Análise Completa"):
    if file_uploaded:
        try:
            ...
            # Adicionado num_ctx=16384 para o modelo não cortar texto!
